...
def worker(args):
    ...
    resmap = { 'inferred': TreeSet(args=args, files = glob(path.join(dirpath,'RAxML_bestTree.*.raxml') ), inferred=True), #, seqs = glob(path.join(dirpath,'{d}')), # separate file for each tree
               'true': TreeSet(args=args, files=StringIO(mstrees), inferred=False) # one filehandle
    }
