...
#@dask.delayed
def process(dname,outdir,outgroup,suffix=''):
    """assumes ALL files in current dir are newick tree files"""
    tops = covs = -1
    results = []
    make_tree = partial(make_tree,outgroup=outgroup)

    try:
        globstr = os.path.join( dname,'*%s'%suffix )
        filenames= glob.glob(globstr)
        if filenames: #no files
            x = db.read_text(
                globstr
            ).map(make_tree)
            covs = x.map(get_cov_all)\
                    .repartition(npartitions = len(filenames)//10+1)\
                    .to_dataframe(meta = COV_SCHEMA)\
                    .to_parquet(
                        os.path.join(outdir,'%s.%s.covs.parquet'%(os.path.basename(dname),suffix)),
                        compute=False,
                        engine='pyarrow'
                    )#.to_delayed()
        
            # NB: df.set_index() is expensive, don't do it
            tops = x.map(get_trio_topologies)\
                    .flatten()\
                    .frequencies()\
                    .repartition(npartitions=len(filenames)//50+1)\
                    .to_dataframe(meta=TOP_SCHEMA)\
                    .to_parquet(
                        os.path.join(outdir,'%s.%s.topos.parquet'%(os.path.basename(dname),suffix)),
                        compute=False,
                        engine='pyarrow'
                    )#.to_delayed()
        ...
        print("\n----------\nerror: %s" % e)
        print('pid',os.getpid(),os.path.join(dname,'%s.%s.covs.parquet'%(pref,suffix)))
...
def main(args):
    ...
    if args.dirlist:
        ...
        dirs = args.indir #[os.path.join(args.indir,x) for x in next(os.walk(args.indir))[1]]
    ...
    for i in range(0,len(dirs),step):
        ...
        results = dask.compute(
            [ process(x,
              ...
              for x in dirs[i:(i+step)] for suf in args.suffixes]
        )
        del results
        gc.collect()
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description='Assumes we have gene trees generated from real dataset.')

    parser.add_argument('--outdir',
                        help='directory to store results files',
                        default='/N/dc2/projects/bkrosenz/deep_ils/results')
    parser.add_argument('--indir',nargs='*', help='directory to search for files',default='/N/dc2/projects/bkrosenz/deep_ils/sims/simphy/SimPhy38')
    ...
    parser.add_argument('--suffixes',nargs='*',default=[''],help='suffices of files to process separately')
    parser.add_argument('--taxa',nargs='*',help='outgroup taxa')
    parser.add_argument('--outgroup',default='0',help='outgroup taxon')
