...
# TODO: add create table entries (id integer primary key, data) to everything so we can save space by using the unique ids as primary keys instead of trees and seqs
...
class SQLio:
    ...
    def set_intersect(self,value_dict):
        ...
        try:
            ...
            print ('found intersect:',param_intersect[:100])
...
#        print('params',self.tree_simulator.param_values)
        ...
        for tree_batch in self.tree_simulator.batch_iter(['id','tree'],batchsize=10000):
...
#            id2align = dict(batch)
            ...
            print('align',align_id,'params',param_names, params_to_simulate)
            for config in params_to_simulate:
                ns = argparse.Namespace(**dict(zip(param_names,config)))
                ns.schema = self.fields
