...
def make_hdf5_file(outpath, algnames, xcols, overwrite=False, append=False):
    ...
    return utils.HDF5Store(
        outpath,
        datasets = ['/%s/x'%a for a in algnames],
        shapes = [(len(xcols),)]*n_algs,
        dtypes = ['f8']*n_algs*2,
...
def main(args):
    ...
    hdf5_store = make_hdf5_file(outpath, algnames, xcols, args.overwrite, args.append)
    ...
    for alg in algnames:
        ...
        try:
            ...
            x = np.array((*chain.from_iterable(summary_list),)) # assume summaries is list of lists
            print('x',x.shape)
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(
        description='''get summary stats for classification/regression 
        on actual (test) data - no y columns.''')
    ...
    parser.add_argument('--testonly',
                        action='store_true',
                        help='''only write test data (no "true" attributes). 
                        not implemented.'''
    )
