...
def get_trees(filepath):
    try:
        with open(filepath) as f:
            itree_values = [ tree_config.make_tree(tree) for tree in f if '(' in tree ]
        return itree_values
...
if __name__=="__main__":
    ...
    itree_files = glob(path.join(args.simpath, 'inferred_trees/*raxml.trees'))
    sim_prefixes = [ rx.findall(x)[0] for x in itree_files ] 
    gtree_files_all = set( path.splitext(path.basename(x))[0] for x in glob(path.join(args.simpath,'trees/*.trees')) )
