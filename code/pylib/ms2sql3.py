...
if __name__=="__main__":
    ...
    filename_prefixes = [m.findall(s)[0] for s in gtree_files]
    join_times = ( [float(f) for f in s.split('_')] for s in filename_prefixes )

    # note: don't use make_tree, it will midpoint-root the outgroup
    trees = (
        utils.Tree( '( ( ( 1:{t1}, 2:{t1} ):{t2_1}, 3:{t2} ):{to_2}, 4:{to});'.format(
            t1 = s[0],
            t2 = s[1],
            to = s[2],
            t2_1 = s[1] - s[0],
