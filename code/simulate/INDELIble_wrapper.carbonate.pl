#! /N/soft/rhel7/perl/gnu/5.24.1/bin/perl
...
# ?? [EVOLVE]
# Diego Mallo 2015
# Modified by Ben Rosenzweig 2018 to ignore already generated files, process files in parallel
use strict;
#GSL
...
while ($content=~m/(\[SIMPHY-PARTITIONS\].*?)((?=\[[A-Z])|$)/sg)
{
	...
	push(@part_model,$3);
	push(@length,$4);
        print "getting model: $1,$2,$3,@part_model\n";
#	$length[-1]=~m/\$\((.*)\)/ and $length[-1]=parse_sampling($1);
	
}
# $total_percentage != 1.0 
abs($total_percentage - 1) > 0.000001 and die "total=$total_percentage. " . "[SIMPHY-PARTITIONS] Tree relative quantity does not add up to one. Please, check your input config file.\n"; 
...
foreach my  $dir (@dirs)
{
	...
	print "\n\n\nTreating gene trees from the replicate $sp_counter\n";
        ...
        # skip files we already wrote
        next if -f $dir . "control.txt";
	...
	my $prev_ptrees=0;
	my $temp_length="";
        print "models: @part_model.  files: @files\n";
        TREE_LOOP : for (my $tree_id=1; $tree_id<= scalar @files; $tree_id++)
	{
		if ($tree_id/$n_trees>$p_trees[$partition_id]+$prev_ptrees+1) #Next partition
		{
			$prev_ptrees+=$p_trees[$partition_id];
			$partition_id+=1;
                        my $rat=$tree_id/$n_trees - $prev_ptrees;                        
                        print "tree id: $tree_id, tid/ntrees=$rat\n";
                        print "new id: $partition_id, @p_trees, trees $p_trees[$partition_id]\n";
                        last TREE_LOOP if $partition_id >= scalar(@part_model);
		}
		...
		{
			die "The parsed model \"$part_model[$partition_id]\" at part $partition_id of the current partition \"$part_names[$partition_id]\"does not correspond with any sampled model\n";
		}
...
if ($is_parallel && $n_threads!=0)
{
	...
	system("ls -d [0-9][0-9][0-9] | parallel -P $n_threads 'cd {} && indelible.carbonate'");
}
else
{
