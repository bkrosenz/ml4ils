...
tree_prefix=ta_${ta}_tb_${tb}_tc_${tc}.$suffix
prefix=ta_${ta}_tb_${tb}_tc_${tc}.${suffix}.prot

echo $prefix

# assume we've alread generated trees with ms
...
$PROG/seq-gen -mWAG \
     -g5 \
     -s$scale -l$seqlen < $DATADIR/trees/$tree_prefix.trees \
...
# python $CODEDIR/utils/distance_matrix.py --treedir $DATADIR/trees/$tree_prefix \
#            --outdir $DATADIR/trees/$tree_prefix \
#            --jobs 1 \
#            --limit $loci;
